...
class Provider extends Component {
  constructor(props){
    super(props)
    const address ='0xfA67ddE98346d6033f3Da0b157b70fe8434a48cE'
    ...
    this.state  = {
      ...
      // Write
      /* ---------------------- */
      get: async ({ key, space, access = 'public', all }) => {
        let value
        try {
          if(space) {
            all 
      ...
      joinThread: async (name, options, space) => {
        try {
          let members = '' ,moderators = ''
          const thread = await this.state.spaces['meshhub'].instance.joinThread(name, options)
      ...
      threadPost: async (thread, message) => {
        try {
          const post = await this.state.threads[thread].instance.post(message)
